...
async function updateRecord(event, openid) {
  ...
  if (updateData.recordTime !== undefined && updateData.recordTime !== null && updateData.recordTime !== '') {
    ...
    if (!Number.isNaN(normalized.getTime())) {
      updateData.recordTime = normalized
    } else {
      delete updateData.recordTime
    }
  }
...
async function getTodaySummary(event, openid) {
  const familyId = await getFamilyId(openid)
  const { currentBabyId, currentBaby } = await getCurrentBabyFast(openid)
...
async function getCurrentBabyFast(openid) {
  const familyId = await getFamilyId(openid)
  ...
  let photoViewUrl = currentBaby.photoUrl || ''

  if (typeof currentBaby.photoUrl === 'string' && currentBaby.photoUrl.startsWith('cloud://')) {
    try {
      const tempRes = await cloud.getTempFileURL({ fileList: [currentBaby.photoUrl] })
      const item = tempRes && tempRes.fileList && tempRes.fileList[0]
      photoViewUrl = (item && item.tempFileURL) ? item.tempFileURL : ''
    } catch (e) {
      photoViewUrl = ''
    }
  }
