...
Page({
  ...
  async loadData() {
    try {
      ...
      const userAvatar = (userInfo && userInfo.avatarUrl) || '/assets/icons/default-avatar.png'

      const babyProfile = this.buildBabyProfile(currentBaby)
      const nextPhoto = babyProfile.photoUrl || '/assets/icons/default-avatar.png'
  ...
  buildBabyProfile(currentBaby) {
    ...
    return {
      ...
      // 首页图片地址与设置页保持一致：优先 photoViewUrl，其次 photoUrl
      photoUrl: currentBaby?.photoViewUrl || currentBaby?.photoUrl || '/assets/icons/default-avatar.png'
    }
  },
  ...
  onBabyImageError() {
    const fallback = '/assets/icons/default-avatar.png'
