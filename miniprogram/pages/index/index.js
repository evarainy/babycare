...
Page({
  data: {
    ...
    todaySummary: { todayCount: 0, totalAmount: 0, formulaAmount: 0, breastMilkAmount: 0, directFeedDuration: 0, directFeedDurationText: '0分钟', foodAmount: 0, foodCount: 0, intervalMinutes: null },
    intervalText: '--',
  ...
  async loadData() {
    try {
      ...
      const directFeedDuration = allRecords
        .filter(r => r.status === 'confirmed' && r.type === 'breastfeeding')
  ...
  buildBabyProfile(currentBaby, userAvatar) {
    ...
    const photoCandidate = currentBaby?.photoViewUrl || currentBaby?.photoUrl || userAvatar || '/assets/icons/default-avatar.png'
    return {
      ...
      photoUrl: this.normalizeImageUrl(photoCandidate, userAvatar)
    }
  },

  normalizeImageUrl(url, fallback) {
    const str = typeof url === 'string' ? url.trim() : ''
    const defaultUrl = fallback || '/assets/icons/default-avatar.png'
    if (!str) return defaultUrl

    if (str.startsWith('/')) return str
    if (str.startsWith('http://') || str.startsWith('https://')) return str
    if (str.startsWith('data:image/')) return str

    // 开发者工具 web 端不支持 wxfile://，避免控制台报 ERR_UNKNOWN_URL_SCHEME
    if (str.startsWith('wxfile://')) return defaultUrl

    // cloud:// 需要先换成临时访问链接；若未转换，使用兜底图避免加载失败噪音
    if (str.startsWith('cloud://')) return defaultUrl
